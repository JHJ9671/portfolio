$(document).ready(function(){
    ...
    $(window).on("scroll",function(){ //메뉴 활성화	
	   ...
	   */
		...
		for(var i=0; i<4;i++){ //해당 스크롤 위치에 대한 메뉴 활성화
		...
		$("section").on("mousewheel",function(event,delta){
			...
			if(delta<0 && ind==cnt){ // 0페이지 이상 혹은 마지막 페이지에서 스클롤을 내릴 때
				var first = $("section:eq(0)").offset().top;
				$("html,body").stop().animate({"scrollTop":first},1500)
			}
			// console.log(delta);
			// 휠 up = +1 , 휠 down = -1 
            //마우스 휠을 올렸을때	
              if (delta > 0 && ind!=0) {  // +1 section의 index가 0이 아닐때
                //변수 prev에 현재 휠을 움직인 section에서 이전 section의 offset().top위치 저장
                 var prev = $(this).prev().offset().top;
                 //문서 전체를 prev에 저장된 위치로 이동
                 $("html,body").stop().animate({"scrollTop":prev},'slow');
                 return false; // 부드럽게 이동
            //마우스 휠을 내렸을때	 
              }else if (delta < 0 && ind!=cnt) { // -1 section의 index가 마지막section이 아닐때
